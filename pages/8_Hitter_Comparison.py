...
_cmp_archetype_map = {}
_cmp_radar_df = get_cached_radar_data(season, player_type="hitter", min_pa=100)
...
_cmp_proj_radar_df = get_cached_projected_radar_data(season, player_type="hitter")
...
if not qp1:
    qp1 = st.session_state.get("hitter_comparison_last_p1", "")
if not qp2:
    qp2 = st.session_state.get("hitter_comparison_last_p2", "")
...
st.session_state["hitter_comparison_last_p1"] = selected_p1
st.session_state["hitter_comparison_last_p2"] = selected_p2
...
if not _cmp_radar_df.empty or not _cmp_proj_radar_df.empty:
    _radar_pcts_1 = get_player_radar_percentiles(_cmp_radar_df, p1["name"], p1["team"], "hitter") if not _cmp_radar_df.empty else None
    _radar_pcts_2 = get_player_radar_percentiles(_cmp_radar_df, p2["name"], p2["team"], "hitter") if not _cmp_radar_df.empty else None
    # Fall back to projected radar for players below threshold
    if _radar_pcts_1 is None and not _cmp_proj_radar_df.empty:
        _radar_pcts_1 = get_player_radar_percentiles(_cmp_proj_radar_df, p1["name"], p1["team"], "hitter")
    if _radar_pcts_2 is None and not _cmp_proj_radar_df.empty:
        _radar_pcts_2 = get_player_radar_percentiles(_cmp_proj_radar_df, p2["name"], p2["team"], "hitter")
    ...
    if _radar_pcts_1 or _radar_pcts_2:
        with st.expander("How does this work?"):
            ...
            st.markdown(
                "**Radar Chart:** Shows how each player compares to every hitter with 100+ plate appearances "
...
def _add_player_annotations(fig, anno_pts):
    ...
    anno_pts: list of dicts with keys: name, x, y, color
    """
    if len(anno_pts) == 2:
        y_vals = [fig.layout.yaxis.range] if fig.layout.yaxis.range else [None]
        # Estimate y-range from all trace data
        all_y = []
...
if len(all_season_pa_rankings) > 0 and (p1["player_id"] is not None or p2["player_id"] is not None):
    ...
    if tl_data[1] or tl_data[2]:
        ...
        # --- Rate stat comparison chart builder ---
        def _build_comparison_rate_chart(rate_prefix, y_label, higher_is_better):
            ...
            # Check if either player has data for this rate stat
            has_data = False
            for pnum in [1, 2]:
            ...
            # Player name annotations (at projection endpoint if available, else last actual)
            anno_pts = []
            for pnum, pdata, color in [(1, p1, p1_color), (2, p2, p2_color)]:
